...
class Orbit:
    ...
    def kepler2rv(self, a: float, e: float, i: float, w: float, omega: float, nu: float) -> np.ndarray:
        u = w + nu
        ...
        return np.array([alpha * r_norm, beta * r_norm, gamma * r_norm, coef1 * alpha + coef2 * alpha_nu,
                         coef1 * beta + coef2 * beta_nu, coef1 * gamma + coef2 * gamma_nu])
    ...
    def rv2kepler(self, rv: np.ndarray) -> np.ndarray:
        ...
        big_omega = atan2(np.dot(l, np.array([0, 1, 0])), np.dot(l, np.array([1, 0, 0])))
