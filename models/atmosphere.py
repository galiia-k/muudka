...
class Atmosphere:
    ...
    def __init__(
        ...
        self.r_c = (
            r_c  # ACS: difference between mass center and geometric center (vector)
        )
        self.S = S  # plate area
    ...
    def momentum(self, x: np.ndarray) -> np.ndarray:
        ...
        J1, J2, J3 = (
            Ev_n * np.cross(Ev, self.r_c),
            Ev_n**2 * np.cross(self.n, self.r_c),
        ...
        return (
            ...
            * np.dot(v_acs, v_acs)
            * self.S
            * ((1 - self.e) * (J1 + self.U_div_V0 * J3) + 2 * self.e * J2)
        )  # in ACS
