...
class Straight_dipole:
    ...
    def get_B(self, x: np.ndarray) -> np.ndarray:
        ...
        B_ics = (
            -self.mu_e
            / r_norm**5
            * (r_norm**2 * self.k_vec - 3 * np.dot(self.k_vec, r) * r)
        )
        return Quaternion(x[9:13]).conjugate.rotate(
            B_ics
        )  # in attached coordinate system
...
class Inclined_dipole:
    ...
    def __init__(self, start_time: datetime, delta: float, lmbd: float):
        ...
        self.k_vec_ecef = np.array(
            [np.sin(delta) * np.cos(lmbd), np.sin(delta) * np.sin(lmbd), np.cos(delta)]
        )
