...
class Magnetometer:
    ...
    def __init__(self, nominal_matrix: np.ndarray, covariance_matrix: np.ndarray, std_e: float, delta_time: float):
        ...
        if np.linalg.matrix_rank(nominal_matrix) < 3:  # Проверка того, что оси не компланарны
            print("Номинальная матрица магнитометра задана неверно")
            sys.exit()

        self.nominal_matrix = nominal_matrix

        matrix_real = np.array([nominal_matrix[i] + np.random.normal(0, std_e, size=3) for i in range(3)])

        self.matrix_nom_real = np.linalg.inv(nominal_matrix) @ matrix_real  # матрица перехода ССК-магнитометр-ССК
